...
export const Route = createLazyFileRoute('/_main/clients/')({
    ...
    <div className="w-full">
        ...
        {isLoading ? (
          ...
          <Table>
            ...
            <TableBody>
              {clients?.data?.length ? (
                clients.data.map(
                  (row: { [x: string]: ReactNode; id: React.Key | null }) => (
                    <TableRow key={row.id}>
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          {clients?.current_page * clients?.per_page} of {clients?.total} row(s)
          selected.
        </div>
